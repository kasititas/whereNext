...
class EventEditPageState extends State<EventEditPage> {
  ...
  void submitForm (Map data) async {
    Firestore.instance.collection("įvykiai")
  ...
  void getData () {
    Firestore.instance.document("įvykiai/" + widget._eventID).snapshots().listen((DocumentSnapshot snapshot) {
      this.setState(() {
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        appBar: new AppBar(title: new Text("Redaguoti įvykį"),),
        body: initalData == null ? new Text("Kraunama...") :
